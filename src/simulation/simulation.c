...
int calculate_avg_tt(struct Simulation_t *self)
{
  ...
  for (i = 0; i < self->num_customers; i++)
  {
    ...
    printf("Customer %d: Arrival Time: %d, Execution Time: %d, Completion Time: %d\n",
           s->c_info[i]->id,
           s->c_info[i]->arr_time,
           s->c_info[i]->exe_time,
           s->c_info[i]->com_time);
    ...
    printf("\tTurnaround Time: %d\n", s->c_info[i]->com_time - s->c_info[i]->arr_time);
    printf("\tResponse Time: %d\n", s->c_info[i]->exe_time - s->c_info[i]->arr_time);
  }
...
void RR_default(Simulation_t *self)
{
  struct scheduler_thread_args *args = malloc(sizeof(struct scheduler_thread_args));
  args->scheduler = self->scheduler;
